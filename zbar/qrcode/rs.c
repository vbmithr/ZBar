...
void
rs_gf256_init (rs_gf256 * _gf, unsigned _ppoly)
{
  unsigned p;
  int i;
...
static unsigned
rs_gmul (const rs_gf256 * _gf, unsigned _a, unsigned _b)
{
...
static unsigned
rs_gdiv (const rs_gf256 * _gf, unsigned _a, unsigned _b)
{
...
static unsigned
rs_hgmul (const rs_gf256 * _gf, unsigned _a, unsigned _logb)
{
...
static unsigned
rs_gsqrt (const rs_gf256 * _gf, unsigned _a)
{
  unsigned loga;
  if (!_a)
    return 0;
...
static int
rs_quadratic_solve (const rs_gf256 * _gf, unsigned _b, unsigned _c,
		    unsigned char _x[2])
{
  unsigned b;
  unsigned logb;
  ...
  c8 = _gf->exp[logc4 << 1];
  g3 = rs_hgmul (_gf,
		 _gf->exp[logb14 + logc] ^ _gf->exp[logb12 +
						    logc2] ^ _gf->exp[logb8 +
								      logc4] ^
		 c8, logb);
  ...
  l2 =
    rs_hgmul (_gf,
	      rs_gmul (_gf, z2, z2) ^ rs_hgmul (_gf, z2,
						255 - (255 / 15)) ^ c0,
	      2 * (255 / 15));
...
/*Solve a cubic equation x**3 + _a*x**2 + _b*x + _c in GF(2**8).
  Returns the number of distinct roots.*/
static int
rs_cubic_solve (const rs_gf256 * _gf,
		unsigned _a, unsigned _b, unsigned _c, unsigned char _x[3])
{
  ...
  if (logw)
	...
	return 0;
      ...
      /*Recover x from w. */
      _x[0] =
	_gf->exp[_gf->log[_gf->exp[logw] ^ _gf->exp[255 - logw]] + logd] ^ _a;
      logw += 255 / 3;
      _x[1] =
	_gf->exp[_gf->log[_gf->exp[logw] ^ _gf->exp[255 - logw]] + logd] ^ _a;
      _x[2] = _x[0] ^ _x[1] ^ _a;
...
static int
rs_quartic_solve (const rs_gf256 * _gf,
		  ...
		  unsigned char _x[3])
{
  ...
  if (_a)
      ...
      if (t)
	{
	  ...
	  nroots =
	    rs_quartic_solve (_gf, 0,
			      rs_hgmul (_gf, _b ^ rs_hgmul (_gf, s, loga),
					logti), _gf->exp[loga + logti],
			      _gf->exp[logti], _x);
	  for (i = 0; i < nroots; i++)
...
static void
rs_poly_zero (unsigned char *_p, int _dp1)
{
  memset (_p, 0, _dp1 * sizeof (*_p));
}

static void
rs_poly_copy (unsigned char *_p, const unsigned char *_q, int _dp1)
{
...
static void
rs_poly_mul_x (unsigned char *_p, const unsigned char *_q, int _dp1)
{
...
/*Divide the polynomial by the free variable, x (shift the coefficients).
  The number of coefficients, _dp1, must be non-zero.*/
static void
rs_poly_div_x (unsigned char *_p, const unsigned char *_q, int _dp1)
{
...
/*Compute the first (d+1) coefficients of the product of a degree e and a
   degree f polynomial.*/
static void
rs_poly_mult (const rs_gf256 * _gf, unsigned char *_p, int _dp1,
	      const unsigned char *_q, int _ep1, const unsigned char *_r,
	      int _fp1)
{
  int m;
  int i;
...
static void
rs_init_lambda (const rs_gf256 * _gf, unsigned char *_lambda, int _npar,
...
static int
rs_modified_berlekamp_massey (const rs_gf256 * _gf,
			      unsigned char *_lambda, const unsigned char *_s,
			      unsigned char *_omega, int _npar,
			      const unsigned char *_erasures, int _nerasures,
			      int _ndata)
{
...
static int
rs_find_roots (const rs_gf256 * _gf, unsigned char *_epos,
  ...
  if (_nerrors <= 4)
    {
      ...
      _nerrors =
	rs_quartic_solve (_gf, _lambda[1], _lambda[2], _lambda[3], _lambda[4],
			  _epos);
...
int
rs_correct (const rs_gf256 * _gf, int _m0, unsigned char *_data, int _ndata,
...
void
rs_compute_genpoly (const rs_gf256 * _gf, int _m0,
  ...
  for (i = 0; i < _ndata - _npar; i++)
      ...
      if (d)
	{
	  ...
	  for (j = 0; j < _npar - 1; j++)
	    {
	      lfsr[j] =
		lfsr[j + 1] ^ rs_hgmul (_gf, _genpoly[_npar - 1 - j], logd);
	    }
	...
	for (d = 0; d < 256; d++)
	  {
		...
		e[mroots] =
		  d ^ rs_hgmul (&gf, c, logx) ^ rs_hgmul (&gf, b,
							  logx2) ^
		  rs_hgmul (&gf, a,
			    gf.log[gf.exp[logx + logx2]]) ^ gf.
		  exp[logx2 << 1];
		if (!e[mroots])
		  r[mroots++] = j;
	      }
