...
static int
text_is_ascii (const unsigned char *_text, int _len)
{
  int i;
  for (i = 0; i < _len; i++)
    if (_text[i] >= 0x80)
...
static int
text_is_latin1 (const unsigned char *_text, int _len)
{
  int i;
  for (i = 0; i < _len; i++)
    {
...
static void
enc_list_mtf (iconv_t _enc_list[3], iconv_t _enc)
{
  int i;
  for (i = 0; i < 3; i++)
    if (_enc_list[i] == _enc)
...
int
qr_code_data_list_extract_text (const qr_code_data_list * _qrlist,
		  ...
		  case QR_MODE_KANJI:
		      ...
		      if (eci < 0)
			{
			  ...
			  /*If the text is 8-bit clean, prefer UTF-8 over SJIS, since
			    ...
			    if (text_is_ascii ((unsigned char *) in, inleft))
			    {
			      enc_list_mtf (enc_list, utf8_cd);
			    }
			  ...
			  for (ei = 0; ei < 3; ei++)
				...
				if (ei < 2 && enc_list[ei] == latin1_cd &&
				    !text_is_latin1 ((unsigned char *) in,
						     inleft))
				  {
				    int ej;
				    for (ej = ei + 1; ej < 3; ej++)
