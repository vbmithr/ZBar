...
/*Applies a 5x5 Wiener filter to the image, in-place, emphasizing differences
   ...
   large.*/
void
qr_wiener_filter (unsigned char * restrict _img, int _width, int _height)
{

  int _width_by_4 = _width / 4;
  int _height_by_4 = _height / 4;
  unsigned *m_buf[8];
  unsigned *sn2_buf[8];
  unsigned char g;
  int x;
  ...
  for (y = -4; y < _height_by_4*4; y++)
      ...
      for (x = -4; x < _width_by_4*4; x++)
	{
	  ...
	  if (y >= 0 && y < _height - 4 && x >= 0 && x < _width - 4)
		...
		{
		  g =
		    _img[QR_CLAMPI (0, y + i, _height - 1) * _width +
			 QR_CLAMPI (0, x + j, _width - 1)];
		  m += g;
		  m2 += g * g;
		}
	  pm[x + 4] = m;
		...
		if (vn3 < sn2)
		  {
		    ...
		    sn2 *= 25;
		    _img[y * _width + x] =
		      QR_CLAMP255 (g + QR_DIVROUND (a, sn2));
		  }
		else
		  _img[y * _width + x] =
		    (unsigned char) (((m << 1) + 25) / 50);
	      }
	  }
    }
  free (sn2_buf[0]);
...
static void
qr_sauvola_mask (unsigned char * restrict _mask, unsigned * restrict _b, int * restrict _nb,
      ...
      for (y = 0; y < _height_by_4*4; y++)
	{
	  ...
	  for (x = 0; x < _width_by_4*4; x++)
	      ...
	      if (d >= 0)
		{
		  ...
		  mms2 =
		    (m2 - mm >> logwindw + logwindh) * (mm >> logwindw +
							logwindh) + 15 >> 4;
...
/*Interpolates a background image given the source and a conservative
  ...
  Returns the average difference between the foreground and the interpolated
   background.*/
static void
qr_interpolate_background (unsigned char * restrict _dst,
			   int * restrict _delta, int * restrict _ndelta,
			   const unsigned char * restrict _img,
			   const unsigned char * restrict _mask, int _width,
			   int _height, unsigned _b, int _nb)
{
  int delta;
  int ndelta;
  delta = ndelta = 0;
  if (_width > 0 && _height > 0)
    {
      ...
      for (logwindw = 4; logwindw < 8 && (1 << logwindw) < (_width + 15 >> 4);
	   logwindw++);
      for (logwindh = 4;
	   logwindh < 8 && (1 << logwindh) < (_height + 15 >> 4); logwindh++);
      windw = 1 << logwindw;
      windh = 1 << logwindh;
      col_sums = (unsigned *) malloc (_width * sizeof (*col_sums));
...
/*Compute the final binarization mask according to Gatos et al.'s
   method~\cite{GPP06}.*/
static void
qr_gatos_mask (unsigned char *_mask, const unsigned char *_img,
	       const unsigned char * restrict _background, int _width, int _height,
	       unsigned _b, int _nb, int _delta, int _ndelta)
{
  ...
  for (y = 0; y < _height_by_4*4; y++)
	...
	_mask[y * _width + x] =
	  (unsigned char) (-(g - _img[y * _width + x] > thresh[g]) & 0xFF);
      }
  /*{
     FILE *fout;
...
void
qr_binarize (unsigned char *_img, int _width, int _height)
{
  unsigned char *mask;
  ...
  qr_gatos_mask (_img, _img, background, _width, _height, b, nb, delta,
		 ndelta);
  free (background);
  free (mask);
}
...
/*A simplified adaptive thresholder.
  This compares the current pixel value to the mean value of a (large) window
   surrounding it.*/
unsigned char *
qr_binarize (const unsigned char * restrict _img, int _width, int _height)
{
  int _width_by_4 = _width / 4;
  int _height_by_4 = _height / 4;
