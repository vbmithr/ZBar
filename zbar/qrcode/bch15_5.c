...
static const unsigned char gf16_exp[31] = {
  1, 2, 4, 8, 3, 6, 12, 11, 5, 10, 7, 14, 15, 13, 9, 1, 2, 4, 8, 3, 6, 12, 11,
    5, 10, 7, 14, 15, 13, 9, 1
};
...
static unsigned
gf16_mul (unsigned _a, unsigned _b)
{
...
static unsigned
gf16_div (unsigned _a, unsigned _b)
{
...
static unsigned
gf16_hmul (unsigned _a, unsigned _logb)
{
...
static int
bch15_5_calc_syndrome (unsigned _s[3], unsigned _y)
{
  unsigned p;
  int i;
  int j;
...
static int
bch15_5_calc_omega (unsigned _o[3], unsigned _s[3])
{
  unsigned s02;
  unsigned tt;
  unsigned dd;
...
static int
bch15_5_calc_epos (unsigned _epos[3], unsigned _s[3])
{
  unsigned o[3];
  int nerrors;
  int d;
  ...
  if (d == 1)
      ...
      for (i = 0; i < 15; i++)
	{
	  ...
	  if (!
	      (gf16_exp[i + i2] ^ gf16_hmul (o[0], i2) ^ gf16_hmul (o[1], i) ^
	       o[2]))
	    {
	      _epos[nerrors++] = i;
	    }
...
unsigned
bch15_5_encode (unsigned _x)
{
  return (-(_x & 1) & 0x0537) ^ (-(_x >> 1 & 1) & 0x0A6E) ^ (-(_x >> 2 & 1) &
							     0x11EB) ^ (-(_x
									  >> 3
									  & 1)
									&
...
main (void)
{
  ...
  for (i = 0; i < 32; i++)
    codes[i] = bch15_5_encode (i);
  for (i = 0; i < 32; i++)
    printf ("0x%04X%s", codes[i], i + 1 < 32 ? "  " : "\n");
  ...
  for (i = 0; i < 0x8000; i++)
      ...
      if (nerrors < 0)
	{
	  ...
	  if (hamming (i, z) <= 3)
	    printf ("Error: 0x%04X should map to 0x%04X\n", i, z);
	}
      else
	{
